...
func writeConsolidatedReport(path string, tplSuite, recSuite *SuiteResult, tplRules []TemplateRule, recRules []RecordRule, dicts []*Dictionary, samples []Sample, records []CRITRecord, now time.Time) error {
	...
	// YAML frontmatter
	b.WriteString("---\n")
	fmt.Fprintf(&b, "title: CRIT Spec Conformance Report\n")
	...
	fmt.Fprintf(&b, "template_rules: %d\n", len(tplRules))
	fmt.Fprintf(&b, "record_rules: %d\n", len(recRules))
	fmt.Fprintf(&b, "dictionaries: %d\n", len(dicts))
...
func main() {
	...
	now := time.Now().UTC()
	if *reportDir != "" {
		reportFile := filepath.Join(*reportDir, "crit-test-report.md")
